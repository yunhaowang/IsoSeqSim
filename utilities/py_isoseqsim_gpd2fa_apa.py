...
def convert_gpd_to_fasta(input_fa_fl,input_gpd_fl,output_fa_fl):
	...
	for line in input_gpd_fl:
		...
		if strand == "-": # minus strand, get reverse complementary sequence
			...
			seq = seq_com[::-1]
			iso_name = iso + "+" + str(int(tss)+1)
		else:
			seq = "".join(up_seq_list)
			iso_name = iso + "+" + str(tts)

		print >>output_fa_fl, ">" + iso_name
...
def do_inputs():
	parser = argparse.ArgumentParser(description="Generate fasta sequence for each isoform. Note: the isoform is renamed as 'isoform ID' + 'polyA site'", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
